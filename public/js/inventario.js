...
async function exportarAExcel() {
    try {
        SistemaBazar.mostrarMensaje('info', 'Preparando exportación masiva...');
        ...
        productos.forEach(producto => {
            ...
            // Estado (lógica simplificada si no tenemos la función completa)
            let estado = 'Normal';
            if (stock <= 0) estado = 'Agotado';
            else if (stock <= (producto.stockMinimo || 5)) estado = 'Bajo';
            
            // CSV escape
            const nombreSafe = (producto.nombre || '').replace(/"/g, '""');
            
            csv += `"${producto.codigo}","${nombreSafe}","${producto.categoria}",${precio},${costo},${stock},${producto.stockMinimo || 5},${valorTotal},"${estado}"\n`;
        });
        ...
        SistemaBazar.mostrarMensaje('exito', `Exportación completada: ${productos.length} items`);
    } catch (error) {
        console.error(error);
        SistemaBazar.mostrarMensaje('error', 'Error en exportación: ' + error.message);
    }
}
