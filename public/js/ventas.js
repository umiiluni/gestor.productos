...
window.agregarAlCarrito = function(productoId, cantidad = 1) {
    ...
    // Lógica para productos por peso/litro
    // Si la cantidad es la por defecto (1) y el producto no es por unidad, preguntar cantidad
    console.log('Verificando tipo unidad:', producto.unit_type, 'Cantidad:', cantidad);
    if (cantidad === 1 && producto.unit_type && ['kg', 'lt', 'mts'].includes(producto.unit_type.toLowerCase())) {
        console.log('Solicitando cantidad para producto peso/litro');
        let inputCantidad = prompt(`Ingrese cantidad para ${producto.nombre} (${producto.unit_type}):`, "1");
        if (inputCantidad === null) return; // Cancelado por usuario
        
        let cantidadIngresada = parseFloat(inputCantidad.replace(',', '.'));
        if (isNaN(cantidadIngresada) || cantidadIngresada <= 0) {
            SistemaBazar.mostrarMensaje('error', 'Cantidad inválida');
            return;
        }
    ...
    if (itemIndex !== -1) {
        ...
        carrito.push({
            ...
            cantidad: cantidad,
            unit_type: producto.unit_type || 'un'
        });
    }
        ...
        itemDiv.innerHTML = `
            ...
            <div style="flex: 1; text-align: center;">
                <div style="margin-bottom: 5px; display: flex; align-items: center; justify-content: center;">
                    ...
                    <input type="number" class="input-cantidad-carrito" data-index="${index}" 
                           value="${item.cantidad}" 
                           min="0.01" 
                           step="${(item.unit_type === 'un' || !item.unit_type) ? '1' : '0.1'}"
                           style="width: 60px; text-align: center; border: 1px solid #ddd; border-radius: 4px; margin: 0 5px; padding: 2px;">
    ...
    document.querySelectorAll('.input-cantidad-carrito').forEach(input => {
        input.addEventListener('change', function() {
            ...
            if (!isNaN(nuevaCantidad) && nuevaCantidad > 0) {
                fijarCantidad(index, nuevaCantidad);
            } else {
                actualizarCarritoUI(); // Revertir cambios inválidos
            }
        });
    });
...
function fijarCantidad(index, cantidad) {
    const productos = JSON.parse(localStorage.getItem('productos') || '[]');
    ...
    if (producto && producto.stock < cantidad) {
        SistemaBazar.mostrarMensaje('error', `Stock insuficiente. Disponible: ${producto.stock}`);
        actualizarCarritoUI(); // Revertir visualmente
        return;
    }
...
function modificarCantidad(index, cambio) {
    const item = carrito[index];
    const nuevaCantidad = parseFloat((item.cantidad + cambio).toFixed(3)); // Evitar errores de redondeo
