...
class PDFProcessor {
    ...
    static extraerProductoDeLinea(linea, formato, config) {
        ...
        if (formato.tipo === 'pipes' || linea.includes('|')) {
            ...
            if (partes.length >= 3) {
                ...
                for (let i = 0; i < partes.length; i++) {
                    ...
                    if (/\$?\s*[\d.,]+\d/.test(partes[i]) && precioIdx === -1) {
                        precioIdx = i;
                    }
                }
        ...
        const patrones = [
            ...
            { regex: /^(\d{2,})\s+(.+?)\s+(\$?\s*[\d.,]+\d)/, grupos: [1, 2, 3], nombre: 'cod-nombre-precio' },
            // Nombre, código, precio
            { regex: /^(.+?)\s+(\d{2,})\s+(\$?\s*[\d.,]+\d)/, grupos: [2, 1, 3], nombre: 'nombre-cod-precio' },
            // Precio, código, nombre
            { regex: /^(\$?\s*[\d.,]+\d)\s+(\d{2,})\s+(.+)/, grupos: [2, 3, 1], nombre: 'precio-cod-nombre' },
             // Caso factura simple: Cantidad (ignorada) Descripcion Precio Total
            { regex: /^\d+\s+(.+?)\s+(\$?\s*[\d.,]+\d)\s+[\d.,]+\d/, grupos: [1, 1, 2], nombre: 'factura-simple' }
        ];
    ...
    static normalizarPrecio(precioStr) {
        ...
        try {
            ...
            if (lastComma > lastDot) {
                // Formato Europeo: 1.200,50 o 1200,50
                // Eliminar puntos (miles) y cambiar coma final por punto (decimal)
                limpio = limpio.replace(/\./g, '').replace(',', '.');
            } else if (lastDot > lastComma) {
                // Formato Inglés: 1,200.50 o 1200.50
                // Eliminar comas (miles)
                limpio = limpio.replace(/,/g, '');
            } else if (lastComma !== -1) {
                // Solo comas: 12,50 -> 12.50
